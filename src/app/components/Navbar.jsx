...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-2 md:top-4 left-0 right-0 z-50 px-1 md:px-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center gap-0.5 md:gap-2">
          ...
          <motion.div
            ...
            animate={{ y: 0, opacity: 1 }}
            transition={{ duration: 0.55, ease: 'easeOut' }}
            className={`backdrop-blur-2xl rounded-full px-1 md:px-8 py-1.5 md:py-2 flex items-center gap-0.5 md:gap-6 transition-all duration-300 overflow-x-auto ${
              isScrolled
                ? isDark ? 'bg-black/80 border border-blue-500/20 shadow-2xl' : 'bg-white/80 border border-blue-500/20 shadow-2xl'
                : isDark ? 'bg-white/5 border border-white/10 shadow-lg' : 'bg-gray-800/10 border border-gray-800/10 shadow-lg'
            }`}
          ...
          <motion.div
            ...
            animate={{ y: 0, opacity: 1 }}
            transition={{ duration: 0.55, ease: 'easeOut' }}
            className={`backdrop-blur-2xl rounded-full px-2 md:px-4 py-1.5 md:py-2 transition-all duration-300 cursor-pointer ${
              isScrolled
                ? isDark ? 'bg-black/80 border border-blue-500/20 shadow-2xl' : 'bg-white/80 border border-blue-500/20 shadow-2xl'
                : isDark ? 'bg-white/5 border border-white/10 shadow-lg' : 'bg-gray-800/10 border border-gray-800/10 shadow-lg'
            }`}
